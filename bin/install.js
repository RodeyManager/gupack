...
function runSwap(cwdir, arg, startText, endText){
    ...
    let loadPrg      = new LoadProgress(startText, endText);
    loadPrg.frames   = cliSpinners.bouncingBar.frames;
    loadPrg.interval = cliSpinners.bouncingBar.interval;
    loadPrg.start();

    let cmd = spawn(T.cmdify('npm'), arg, {stdio: 'pipe', cwd: cwdir});
    cmd.stdout.pipe(loadPrg.ui.log);
