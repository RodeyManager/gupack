...
function _createProject() {
  ...
  function _next(template) {
    ...
    // 跳过缓存 或 缓存不存在则下载
    if (
      !T.fs.existsSync(tempPath) ||
      !T.fs.existsSync(T.Path.resolve(tempPath, 'template'))
    ) {
...
function autoInstallPackages() {
  process.nextTick((e) => {
    ...
    let packages =
      Object.keys(devDependencies).length + Object.keys(dependencies).length;
    // 安装对应插件
    let endText = 'Finished OK =^_^= (^_^) =^_^= \n\r';
    endText += T.msg.green(
      `${T.msg.green('√')} Installed ${packages} pachages \n\r`
    );
    let startText = T.msg.yellow(
      `→ installing npm pachages [${packages}] ......`
    );
