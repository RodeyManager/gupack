...
const gupack = program
     ...
     */
    .option('new', repx('<projectName*> 创建项目; '
    ...
    .option('build', repx('[<projectName>]  [<options>]编译项目; '
    ...
    // start dev server
    .option('start', repx('启动内置Node静态服务器; '
    ...
    .option('clean', repx(' 清空编译路径下的所有文件; '
        + exp('\n\t -T, --template 清除所有模板'
    ...
    .option('addTemplate', repx('<templateName*> <templateGitUrl(注：github/repo)*> 添加项目模板; '
        + exp('\n\t --download-template 下载模板')), function(){
        require('./add').addTemplate();
    })
    .option('removeTemplate', repx('<templateName>删除指定项目模板; '
