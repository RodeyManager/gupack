...
class TaskNode {
  ...
  constructor(config, gupack) {
    ...
    this.merge = false;
    this.stream = null;
    this.$ = this.loadPlugin = $;
    // 编译前
    this.before = null;
    // 编译后
    this.after = null;
    // 编译当前任务前清理对应输出目录
    this.beforeClean = false;
  ...
  init() {
    //watcher
    if (!this.noWatch || this.watch) this.watch = this.getTaskWatch();
  ...
  getTaskFunction(name, done, options = {}) {
    // const hrStart = process.hrtime();
    // const isParallel = options['type'] === 'parallel';
    // isParallel && this.gulp.emit('start', { name });
    // 加载的gulp插件,
    ...
    if (util.isFunction(loaders)) {
      this.stream = loaders.call(null, this.stream, done) || this.stream;
      // return this.stream;
    }
      ...
      Object.keys(loaders).forEach((loaderName) => {
        ...
        if (!plugin) {
          plugin = (() => {
            ...
            if (!pp) {
              T.log.error(
                `'${loaderName}' plugin is not loaded, Stream content is not supported`
              );
            }
  ...
  excutePublish() {
    if (
      !util.isEmptyObject(this.gupack.statics) &&
      this.gupack.statics !== false
    ) {
  ...
  excuteDest() {
    if (this.dest) {
      // if (!T.fs.existsSync(this.dest)) {
      //     T.fsa.mkdirsSync(this.dest);
      // }
      this.stream = this.stream.pipe(
        this.gulp.dest(this.dest || this.buildDir)
      );
    }
  }
