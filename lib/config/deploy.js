...
class Deploy extends EventEmitter {
    ...
    initPath() {
        if (util.isString(this.localPath) && this.localPath.length > 0) {
            ...
            //                     : [ this.localPath ];
            this.localPath = (!T.isAbsolutePath(this.localPath) && [T.Path.resolve(process.cwd(), this.localPath)]) || [
                this.localPath
            ];
    ...
    // connect
    connectSFTP() {
        if (this.sftp) return this.startUpload(this.sftp);
            ...
            vfs
                ...
                .on('close', () => {
                    ...
                    util.isFunction(this.onUploadedComplete)
                        ...
                        : T.log.yellow(
                              `√ [${this.host || ''}] ${this.fileCount} files uploaded OK =^_^= (^_^) =^_^= !!!`
                          );
                });
        });
    }
        ...
        stream.on('close', err => {
            if (err) {
                ...
                util.isFunction(this.onUploadedFileSuccess)
                    ...
                    : T.log.green(
                          `√ [${T.getTime()}] uploaded '${realPath}', after ${T.msg.yellow(size / 1000 + ' kb')}`
                      );
            }
            next(null, file);
        });
    ...
    onEnd() {
        ...
        this.sftp = null;
        if (!this.finished) this.emit('error', new Error('SFTP abrupt closure'));
    }
