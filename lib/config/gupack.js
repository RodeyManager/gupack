...
class Gupack extends EventEmitter {
    ...
    initTasks() {
        if (!util.isObject(this.buildTasks)) {
            T.log.error(
                `× ${new TypeError('参数 buildTask 未找到或类型错误（buildTask必须是一个对象Object）' + '\n× 请您检查项目的gupack-config.js文件配置')}`
            );
            return false;
        }
        ...
        for (let key in tasks) {
            if (tasks.hasOwnProperty(key)) {
                ...
                const taskNode = !util.isFunction(task) ? new TaskNode(task, this) : new TaskNodeFun(task);
                taskNode.name = key;
    ...
    initWatch() {
        ...
        if (util.isArray(this.watch) || util.isString(this.watch) || util.isObject(this.watch)) {
            let watchSource = [],
                ts = null;
            ...
            this.gulp.task('build._watch', () => {
                this.gulp.watch(watchSource, ts || this.tasks);
            });
            //添加监听任务
    ...
    addEvent() {
        ...
        this.gulp.on('task_stop', e => {
            ...
            if (tasks.indexOf(name) > -1 && logCache.indexOf(name) === -1 && isRun) {
                T.log.green(`√ [${T.getTime()}] Finished '${e.task}', after ${T.msg.yellow(duration)}`);
                logCache.push(name);
                this.emit('finish_task', { task: e.task, name, duration });
            }
        });
        ...
        this.gulp.on('stop', e => {
            ...
            this.emit('finish_all_task', { event: e, message: finishMsg });
        ...
        this.gulp.on('task_err', e => {
            ...
            T.log.error(`'${e.task}' errored after ${prettyTime(e.hrDuration)} \n\r ${Gupack.formatError(e)}`);
        });

        this.gulp.on('task_not_found', err => {
            self.server && self.server.stop();
            T.log.error(`Task \'' + err.task + '\' is not found \n\r Please check the documentation for proper gupack-config file formatting`);
        });

        function _doneNext() {
            this.emit('finish_all_task_next_before', { gupack: this });
            const deploies = extend(true, [], this.deploies);
            if (this.isDeploy) {
                if (deploies.length > 0) {
                    T.log.yellow(`→ has deploys. but now skip deploy(arg: --skip-deploy).`);
                }
                return process.exit(0);
            }
            ...
            this.deployManager.startDeploy(this.watch);
            this.startServer();
            this.emit('finish_all_task_next_after', { gupack: this });
        }
    }
    ...
    runDeploy(isSkipBackup) {
        this.deployManager.startDeploy(isSkipBackup);
    }
