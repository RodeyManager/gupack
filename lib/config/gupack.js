...
class Gupack extends EventEmitter {
    ...
    initTasks() {
        if (!util.isObject(this.buildTasks)) {
            T.log.error(`× ${new TypeError('参数 buildTask 未找到或类型错误（buildTask必须是一个对象Object）' + '\n× 请您检查项目的gupack-config.js文件配置')}`);
            return false;
        }
        ...
        this.tasks = Object.keys(this.buildTasks).map(taskName => {
            ...
            // const relies = this.buildTasks[taskName]['rely'];
            // if (reies && relies.length > 0) {
            //     this.gulp.task(taskName, relies, done => {
            //         return !!task.run === true ? task.getTaskFunction(done) : done();
            //     });
            // } else {
