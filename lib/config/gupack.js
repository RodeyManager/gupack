...
const
    //是否启动本地服务
    isStartServer   = 's' in T.argv;

class Gupack {

    constructor(config, gulpInstance){

        this.env = T.argv['e'] || 'local';
        this.project = T.argv['p'] || T.argv['project'];
        this.sourceDir = 'src';
        this.buildDir = 'build';
        this.indexFile = '';
    ...
    initPaths(){
        this.basePath = T.argv['cwdir'] || process.cwd();
        this.root = this.basePath;
        if(!T.isAbsolutePath(this.sourceDir)){
            // if(T.isInSourcePath()){
            //     this.sourceDir = T.Path.resolve(this.basePath, '');
            // }else{
        ...
        if(!T.isAbsolutePath(this.buildDir)){
            this.buildDir = T.Path.resolve(this.basePath, this.buildDir || 'build');
        }
    }
    ...
    setTask(){
        ...
        tns.forEach(tn => {
            ...
            task.dest && this.cleans.push(task.dest);
        });
    ...
    startServer(){
        process.nextTick(() => {
            if(!this.runIndex){
                if(isStartServer){
                    this.server = require('../../gupackServer')(this);
                }
            }
            this.runIndex++;
        });
    ...
    startDeploy(){
        ...
        if(this.deploy){
            let isAllDone = false;
            if(this.deploy instanceof Deploy){
                this.deploy.start();
                // this.on('done', (e) => {
                //     console.log('Done.... ', e);
                // });
                ...
                this.deploy.forEach(d => {
                    d && d instanceof Deploy && d.start();
                });
            }
        }
        // this.deploy && this.deploy.start();
    }

    setDefaultTask(){
        this.gulp.task('default', taskSequence.apply(this.gulp, this.tasks));
    }
    ...
    cleanBuildDir(cb){
        ...
        T.fsa.remove(T.Path.join(this.buildDir, '**/*'), (err) => {
            if(err){
                T.log.red('× clean failed: ' + err.toString());
                process.exit(1);
            }else{
                let time = (Date.now() - startDate) * .001;
