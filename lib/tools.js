...
let tools = {
  ...
  getConfig: function (cwdir) {
    ...
    if (cfgFile) {
      configFile = path.isAbsolute(cfgFile)
        ? cfgFile
        : path.resolve(projectPath, cfgFile);
    }
  ...
  getTime: function () {
    let date = new Date();
    return `${fn(date.getHours())}:${fn(date.getMinutes())}:${fn(
      date.getSeconds()
    )}`;
...
for (let name in logs) {
  tools['text'][name] = function (msg) {
    return logs[name] + msg;
  };
}
