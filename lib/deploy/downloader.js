...
class Downloader extends EventEmitter {
    ...
    download() {
        ...
        this.ssh2.sftp((err, sftp) => {
            if (err) throw err;
            this.sftp = sftp;
            this.emit('start', { message: '→ Download start ...... ', backup: this });
    ...
    _done(message, state, directory) {
        this.emit('done', { message: message, state: state || 200, directory, backup: this });
    }

    _error(message, state, directory) {
        this.emit('error', { message, state, directory, backup: this });
