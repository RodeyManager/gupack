...
class Deploy extends EventEmitter {
  ...
  initPath() {
    if (util.isString(this.localPath) && this.localPath.length > 0) {
      ...
      T.log.error(
        '× LocalPath Error: localPath is not found ( localPath can be String or Array => vinyl-fs module )'
      );
    }
  }
  ...
  initRemotePath() {
    if (!this.remotePath) {
      T.log.error(
        '× RemotePath Error: remotePath is not found ( remotePath can be String )'
      );
    }
  ...
  _onUploaded(payload) {
    ...
    if (error) {
      ...
      return this._isProgressLog
        ? this.loading.text(message)
        : T.log.green(T.msg.red(message));
    }
    this._writeLogFile(message);
    util.isFunction(this.onUploadedFileSuccess)
      ? this.onUploadedFileSuccess.call(this, {
          file,
          realPath,
          size
        })
  ...
  _onDone(payload) {
    ...
    let iText = this._isIncrementType
      ? `√ [${this.host}] Deploy Status: ` + T.msg.cyan(status)
      : '';
    let dText =
      iText +
      T.msg.green(
        `√ [${this.host}] ${
          this._isFullType ? fileCount + ' ' : ''
        }files uploaded OK =^_^= (^_^) =^_^= !!!`
      );
  ...
  _stopText(duration) {
    return `√ [${
      this.host
    }] Deploy done =^_^= (^_^) =^_^= !!!, after ${T.msg.yellow(
      (duration / 1000).toFixed(2) + ' s'
    )} \n`;
  }
  _uploadedText(realPath, size) {
    return `√ [${T.getTime()}] uploaded '${realPath}', ${T.msg.yellow(
      size / 1000 + ' kb'
    )}`;
  }
  _writeLogFile(txt) {
