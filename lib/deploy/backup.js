...
class Backup extends EventEmitter {
    ...
    initStoreFile() {
        this.backupFile = T.Path.resolve(this.deploy ? this.deploy.basePath : process.cwd(), 'backup.json');
    ...
    initZIP() {
        ...
        // 备份名称中不能包含空格或特殊字符
        if (/[^\w_-]/.test(name)) {
            T.log.error(`× [${T.getTime()}] backup name field not has special characters or spaces can be included`);
        }
    ...
    _removeBacks(backName) {
        ...
        if (backup) {
            ...
            T.log.yellow(`√ [${backup.server.host}] backup '${backName}]' is remove done `);
            this.emit('remove_done', { backName, backup });
        }
    }
    ...
    _startLocalBackup() {
        ...
        const downloader = new Downloader(this.deploy.remotePath, this.zipPath, this.auths, this.filters);
        downloader.on('start', () => {
        ...
        downloader.on('error', ({ message, state, dir }) => {
            message = `${this.auths.host} ${message}`;
            this._isLogAll ? T.log.error(message) : this.loadingORA.text(T.msg.red(message));
        });
    ...
    _backup() {
        ...
        this.ssh2.exec(command, (err, stream) => {
            if (err) {
                ...
                T.log.error(`× [${host}] Remote server exec command failed \n\t ${err.message}`);
            }
            stream.on('close', err => {
                if (err) {
    ...
    _onDone({ message, state, directory }) {
        ...
        if (state !== 200) {
            ...
            T.fs.existsSync(this.zipPath) && this.deploy.remotePath === directory && T.fsa.removeSync(this.zipPath);
        } else {
            this.setBackup();
        }
        // 打印相关信息
    ...
    getBackups() {
        ...
        try {
            ...
            T.log.error(`× [${T.getTime()}] '${this.backupFile}' is not found \n ${e.message}`);
        }
        return backs || {};
    }
