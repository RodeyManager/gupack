...
class Uploader extends EventEmitter {
    ...
    stream.on('close', () => {
      vfs
        ...
        .on('close', () => {
          ...
          const status = `
                    server info:  host: ${this.host}; user: ${
            this.auths.username
          }
  ...
  checkDir(file, next) {
    ...
    directories = directories.filter(
      (d) => d !== this._getRelativePath(this.remotePath)
    );
  ...
  _downloader(file, next, realPath) {
    ...
    stream.on('close', (err) => {
      ...
      if (localHash !== remoteHash) {
        message = T.msg.green(
          `√ [${T.getTime()}] check hashContent ( Modified ), filename: '${filename}' `
        );
        this.emit('uploaded', {
          message,
          file,
        ...
        this.modCount++;
        message = T.msg.green(
          `√ [${T.getTime()}] check hashContent ( No Modify ), filename: '${filename}' `
        );
        this.emit('uploaded', {
          message,
          file,
  ...
  _uploader(file, next, realPath) {
    ...
    this.sftp.fastPut(file.path, realPath, { mode: '0777' }, (err) => {
      if (err) {
        this.errCaches[realPath] = true;
        const message = T.msg.red(
          `× '${realPath}' upload error: ${err.message}`
        );
        this.emit('uploaded', {
          message,
          file,
          realPath,
          size,
          error: T.msg.red(`× ${message}: ${err.message}`)
        });
      } else {
        this.fileCount++;
        this.emit('uploaded', {
          message: this._uploadedText(realPath, size),
          file,
          realPath,
          size
        });
    ...
    // stream.on('close', (err) => {
    //   console.log('_upload close', err, this.fileCount, realPath);
